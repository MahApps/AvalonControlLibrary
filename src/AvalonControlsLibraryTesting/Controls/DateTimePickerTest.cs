    ...
    public class DateTimePickerTest
    {
        ...
        public void TestSyncronizationOFPickers()
        {
            AvalonTestRunner.RunInSTA(delegate
            {
                ...
                picker.DateTimeSelected = new DateTime(1, 1, 1, 2, 2, 2);
                Assert.AreEqual(picker.DateTimeSelected.Value.Date, datePicker.CurrentlySelectedDate.Date, "Invalid date set");
                Assert.AreEqual(picker.DateTimeSelected.Value.TimeOfDay, timePicker.SelectedTime, "Invalid time set");
                ...
                Assert.AreEqual(testDate, picker.DateTimeSelected.Value.Date, "Invalid date set from date picker");

                TimeSpan testTime = new TimeSpan(1, 2, 1);
                timePicker.SelectedTime = testTime;
                Assert.AreEqual(testTime, picker.DateTimeSelected.Value.TimeOfDay, "Invalid time set from time picker");
            });
        }
    }
}
